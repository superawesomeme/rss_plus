...
class feed_edit_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('header', 'rsseditfeedheader', get_string('feed', 'block_rss_plus'));

        $mform->addElement('text', 'url', get_string('feedurl', 'block_rss_plus'), array('size' => 60));
        $mform->setType('url', PARAM_URL);
        $mform->addRule('url', null, 'required');

        $mform->addElement('checkbox', 'autodiscovery', get_string('enableautodiscovery', 'block_rss_plus'));
        $mform->setDefault('autodiscovery', 1);
        $mform->setAdvanced('autodiscovery');
        $mform->addHelpButton('autodiscovery', 'enableautodiscovery', 'block_rss_plus');

        $mform->addElement('text', 'preferredtitle', get_string('customtitlelabel', 'block_rss_plus'), array('size' => 60));
        $mform->setType('preferredtitle', PARAM_NOTAGS);

        if ($this->caneditshared) {
            $mform->addElement('selectyesno', 'shared', get_string('sharedfeed', 'block_rss_plus'));
            $mform->setDefault('shared', 0);
        }

        $submitlabal = null; // Default
        if ($this->isadding) {
            $submitlabal = get_string('addnewfeed', 'block_rss_plus');
        }
    ...
    function validation($data, $files) {
        ...
        if ($rss->error()) {
            $errors['url'] = get_string('errorloadingfeed', 'block_rss_plus', $rss->error());
        } else {
...
$PAGE->set_url('/blocks/rss_plus/editfeed.php', $urlparams);
...
if ($mform->is_cancelled()) {
    ...
    if ($isadding) {
        $DB->insert_record('block_rss_plus', $data);
    } else {
        $data->id = $rssid;
        $DB->update_record('block_rss_plus', $data);
    }
    ...
    if ($isadding) {
        $strtitle = get_string('addnewfeed', 'block_rss_plus');
    } else {
        $strtitle = get_string('editafeed', 'block_rss_plus');
    }
